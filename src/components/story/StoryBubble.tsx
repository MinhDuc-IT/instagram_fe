...
export default function StoryBubble({ group, onClick }: Props) {
  ...
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1">
      <div className={`w-16 h-16 rounded-full p-0.5 ${hasUnseen ? 'bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-500' : 'bg-gray-300 dark:bg-gray-700'}`}>
        <img
      ...
      <span className="text-xs truncate w-16 text-center dark:text-gray-200">{user.userName}</span>
    </button>
  )
}
